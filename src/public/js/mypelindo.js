...
buttonSync.addEventListener('click', async (e) => {
  e.preventDefault();
  buttonSync.textContent = '';
  buttonSync.insertAdjacentHTML(
    'beforeend',
    `
...
function getOSTypeAndVersion() {
  ...
  if (/android/i.test(userAgent)) {
    ...
    osVersion = match ? match[1] : 'Unknown Version';
  } else if (/iPhone|iPad|iPod/i.test(userAgent)) {
    osType = 'iOS';
    const match = userAgent.match(/OS\s([0-9_]+)/);
    osVersion = match ? match[1].replace(/_/g, '.') : 'Unknown Version';
...
const osInfo = getOSTypeAndVersion();
inputDevice.value = `${osInfo.osType} ${osInfo.osVersion}`;
